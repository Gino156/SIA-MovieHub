document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('latest_documentary_data.php')
      .then(response => {
        if (!response.ok) {
  ...
  function handleRatingChange(movieId, rating) {
    fetch('rate_documentarymovie.php?documentaryId=' + movieId + '&rating=' + rating)
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  function fetchSavedMovies() {
    fetch('fetch_saved_documentaries.php')
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
      ...
      .then(savedMovieIds => {
        ...
        savedMovieIds.forEach(movieId => {
          fetch(`get_documentary_details.php?movieId=${movieId}`)
            .then(response => {
              if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
              }
