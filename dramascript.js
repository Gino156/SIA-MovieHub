document.addEventListener('DOMContentLoaded', function() {
  ...
  function handleRatingChange(movieId, rating) {
    fetch('rate_dramamovie.php?dramaId=' + movieId + '&rating=' + rating)
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
    ...
    fetch('fetch_saved_dramas.php')
      ...
      .then(savedMovieIds => {
        ...
        savedMovieIds.forEach(movieId => {
          fetch(`get_drama_details.php?movieId=${movieId}`)
            .then(response => {
              if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
              }
